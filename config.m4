...
if test "$PHP_DOTNET_FFI" != "no"; then
  ...
  SEARCH_FOR="lib/coreclr_ctlpp.h"  # you most likely want to change this
...
dnl 
  ...
  PHP_ADD_LIBRARY_WITH_PATH("coreclr_ctlpp", $DOTNET_FFI_DIR/$PHP_LIBDIR, DOTNET_FFI_SHARED_LIBADD)
  PHP_SUBST(DOTNET_FFI_SHARED_LIBADD)
  PHP_NEW_EXTENSION(dotnet_ffi, dotnet_ffi.c, $ext_shared,, -DZEND_ENABLE_STATIC_TSRMLS_CACHE=1)
fi
