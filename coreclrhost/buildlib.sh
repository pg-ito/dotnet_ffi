...
LIB_DIR="lib"
g++ ${DEBUG_BUILD_FLAG} -O2 -fPIC -c coreclr_ctlpp.cpp -ldl
ar rcs ${LIB_NAME}.a coreclr_ctlpp.o
cp -fpav coreclr_ctlpp.h ../${LIB_DIR}

g++ ${DEBUG_BUILD_FLAG} -O2 --shared -fPIC -o ${LIB_NAME}.so coreclr_ctlpp.cpp -ldl
...
if [ "${1}" == "debug" ];then
  g++  -ggdb -fsanitize=leak -fno-omit-frame-pointer -static-liblsan -static-libgcc ${DEBUG_BUILD_FLAG} -O2 -fPIC -o ${LIB_NAME}_app coreclr_ctlpp.cpp -ldl
  ${DEBUG_APP_CMD}
fi
